...
public class GrowingChamberScreenHandler extends ScreenHandler {
    private final Inventory inventory;
    private final PropertyDelegate propertyDelegate;
    public final GrowingChamberBlockEntity blockEntity;
    public GrowingChamberScreenHandler(int syncId, PlayerInventory inventory, BlockPos pos) {
        this(syncId, inventory, inventory.player.getEntityWorld().getBlockEntity(pos), new ArrayPropertyDelegate(2));
    }

    public GrowingChamberScreenHandler(int syncId, PlayerInventory playerInventory,
                                       BlockEntity blockEntity, PropertyDelegate arrayPropertyDelegate) {
        super(ModScreenHandlers.GROWING_CHAMBER_SCREEN_HANDLER, syncId);
        this.inventory = ((Inventory) blockEntity);
        this.blockEntity = ((GrowingChamberBlockEntity) blockEntity);
        ...
        this.addSlot(new Slot(inventory, 1, 113, 34));
    ...
    public ItemStack quickMove(PlayerEntity player, int invSlot) {
        ...
        if (slot != null && slot.hasStack()) {
            ...
            if (originalStack.isOf(Items.BONE_MEAL)) {
